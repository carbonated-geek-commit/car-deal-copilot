...
undefined · integrator · ADR-005 applied (evaluateOffer now returns FlagEvaluation { flags, unevaluable }: flags with missing required inputs are not emitted and surface as unevaluable — never defaulted to zero; payment_packing evaluable from term alone, over_walkaway requires sale_price, rate_markup requires apr+qualified_apr, junk_fee always evaluable; tests adapted + ADR-005 suites added, 55/55 green, tsc green)

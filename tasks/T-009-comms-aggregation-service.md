...
2026-08-07 · tester · test suite published (services/comms/test/** — 5 suites, 41 tests, fixtures per design §7); stage test → validate; covers design interfaces AND error paths: ack-then-queue table (200 = enqueued pre-heavy-work, enqueue failure = the one 503 with provider-retry recovery, parse failure/throwing adapter = quarantine + 200, D2 dedupe on redelivered garbage); idempotency §5.4 belt+braces (webhook redelivery before/after processing, redelivered downstream events, adversarial different-key duplicate blocked by fill-once, deterministic thread creation); identity routing adversarial (unknown identity → unrouted held-whole never guessed, near-miss/missing-country-code non-match, two deals same dealer isolated, burned deal identity re-bind routes to new deal with burned history untouched, bind/resolve normalization identity); extraction attachment + §6 rollup (ADR-005 partial without sale_price verbatim + never zero, per-field accumulation, Message.timestamp not processing order, order-independence, empty fees[] never erases, flags always []); call flow (consent hook seam, stubbed transcription, transcript-unavailable → bounded retries → DLQ envelope intact with call still threaded, no body/recording_url ever); append-only history (accumulation, deep-snapshot read model, fill-once transcript/extracted_offer, recording_url sweep); npx vitest run services/comms 41/41 green, tsc -p services/comms clean, full root suite 493 passed / 1 skipped — no findings against the build, D4 lead ADR remains the open merge gate
